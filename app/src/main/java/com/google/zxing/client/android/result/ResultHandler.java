...
public abstract class ResultHandler {
  ...
  final void sendEmailFromUri(String uri, String email, String subject, String body) {
	    ...
	    String Firstname = ((CaptureActivity) this.getActivity()).getFirstname_email();
	  	Intent emailIntent = new Intent(android.content.Intent.ACTION_SEND_MULTIPLE);
	    emailIntent.setType("text/plain");
	    emailIntent.putExtra(android.content.Intent.EXTRA_EMAIL, 
	        new String[]{email});
	    emailIntent.putExtra(android.content.Intent.EXTRA_CC, 
	        new String[]{"varanasisai@gmail.com"});
	    ...
	    String directoryPath = Environment.getExternalStorageDirectory() + "/Wipro_QR/"+date+"/"+email+"/";
	    File filePath;
	    String filePath_str = null;
	    ArrayList<Uri> uris = new ArrayList<Uri>();    
	    
	    //convert from paths to Android friendly Parcelable Uri's
	    for (int file_count = 1; file_count <= 3; file_count++) {
    		filePath_str = directoryPath+Firstname+"_pv_"+file_count+".png";
    		// Check if the file is present if not exit  		
    ...
    ArrayList<Uri> uris = new ArrayList<Uri>();
  	...
  	for (int file_count = 1; file_count <= 3; file_count++) {
   		filePath_str = directoryPath+"pv_"+file_count+".jpg";
   		// Check if the file is present if not exit    		
   		filePath = new File(filePath_str);
   		if (filePath.exists()) {
   		  	//Do action
    	...
    	putParcelableArrayListExtra(intent, Intent.EXTRA_STREAM, uris);
    	launchIntent(intent);
    */
...
private void putParcelableArrayListExtra(Intent intent, String extraStream,
		ArrayList<Uri> uris) {
...
@SuppressWarnings("unused")
private void putExtra(Intent intent, String extraStream, Uri emailUri) {
	// TODO Auto-generated method stub
	  if (emailUri != null) {
	      intent.putExtra(extraStream, emailUri);
	    }
...
final void shareBySMS(String contents) {
  ...
  final void dialPhoneFromUri(String uri) {
    launchIntent(new Intent(Intent.ACTION_CALL, Uri.parse(uri)));
  }
